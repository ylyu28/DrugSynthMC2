...
class NMCS:
    ...
    def playout(self, protein, st: State, heuristic_w): # playout, which employs softmax, used in level=1 nmcs
        ...
        if State.CONSIDER_NON_TERM or st.terminal():
            best_state_score = best_state.kd_score(protein)
    ...
    def nmcs(self, protein, st: State, level, heuristic_w, verbose: bool):
        ...
        while not st.terminal(): # runs until the state is terminal (no legal moves are left)
            ...
            for mv in moves:
                ...
                new_st_score = new_st.kd_score(protein)
                new_st_lip = new_st.lipinskiness()

                writeline(str(new_st.smile_to_smile(new_st.SMILE))+ " "+ str(new_st_score) +"\n", f"{self.registerName}_dock" )
                ...
                #         writeline(str(time.time() - self.start_time)+ " " + st_smile + " " + str(new_st_score) + " " + "\n", f"{self.registerName}_sc" )
                #     return new_st
            ...
            writeline(str(time.time() - self.start_time)+ " " + st_smile + " " + str(best_state_lip) + " " + str(best_state_score) + " "+ "\n", f"{self.registerName}_sc" )
        
        if State.CONSIDER_NON_TERM:
            return best_state
        if best_state_score != 1000.0:
            writeline(str(time.time() - self.start_time)+ " " + st.smile_to_smile(st.SMILE) + " " + str(best_state_lip) + " " + str(best_state_score) +  "\n", f"{self.registerName}" )
        return st
...
def launch_nmcs(protein, init_st: State,level, heuristic_w, verbose, timeout, register_name):
    ...
    st = expe.nmcs(protein, init_st, level, heuristic_w, verbose)
    return st 
