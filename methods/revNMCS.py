...
class revNMCS:
    ...
    def nmcs(self, protein, st: State, level, heuristic_w, verbose: bool, initial_level):
        ...
        while not st.terminal(): # runs until the state is terminal (no legal moves are left)
            ...
            for mv in moves:
                ...
                if level <= 1:
                    ...
                    new_st = self.nmcs(protein, new_st, level - 1, heuristic_w, verbose, initial_level)
...
def launch_nmcs(protein, init_st: State,level, heuristic_w, verbose, timeout, register_name):
    ...
    st = expe.nmcs(protein, init_st, level, heuristic_w, verbose, level)
    return st
