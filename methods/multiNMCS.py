    ...
    def nmcs(self, protein, st: State, level, heuristic_w, verbose: bool):
        ...
        while not st.terminal(): # runs until the state is terminal (no legal moves are left)
            ...
            for mv in moves:
                ...
                if (level > 1 and new_st_kd < best_state_kd) or (level <=1 and new_st_lip > best_state_lip) : # update best state if new state is better
                    best_state = new_st
        ...
        if best_state_kd != 1000 and best_state_lip != -2:
            writeline(str(time.time() - self.start_time)+ " " + st.smile_to_smile(st.SMILE) + " " + str(best_state_lip) + " "+ str(best_state_kd) + "\n", f"{self.registerName}" )
        return st
